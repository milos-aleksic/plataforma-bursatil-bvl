...
import { groupByTicker, filterList, orderList } from './cleanDataHelper';
    ...
    getLiveList ({ commit }) {
        axios({
            ...
            .then(response => {
                ...
                // commit('SET_FILTERED_LIST', dataReady.items)
                // commit('ORDER_LIST', dataReady.items)
        })
    },
...
const mutations = {
    SET_STARTER(state, list) {
        // console.log('SET_STARTER', list)
        state.starterList = list
    },
    SET_LIVE(state, liveList) { // Updates STARTER
        // console.log('SET_LIVE', liveList)

        const actualList = state.starterList

        Object.keys(liveList).map((tickerCode) => {
            if (actualList[tickerCode] !== undefined) {
                Object.assign(actualList[tickerCode], liveList[tickerCode])
            }
        })

        state.starterList = actualList
        // state.dispatch("filterList");
        state.filterList
    ...
    SET_FILTERED_LIST(state, list) { // List of all properties received from BVL API, not ll items have all of them
        // console.log('SET_FILTERED_LIST', list)
